...
const DEFAULT_STATUS = getMockUserStatus();
...
export default function TeamNetwork() {
  ...
  const mockLevelData = dynamicLevelData ?? DEFAULT_LEVEL_DATA;

  const dynamicUserStatus = useMemo(() => {
    if (!network) return DEFAULT_STATUS;
    const base = getMockUserStatus();
    const qualifiedUsdScaled = Math.round(
      (Number.isFinite(teamVolumeQualifiedUsd) ? teamVolumeQualifiedUsd : 0) *
        1e8
    ).toString();
    return {
      ...base,
      directChildrenCount: String(directMembers.length),
      qualifiedVolumeUSD: qualifiedUsdScaled,
    };
  }, [network, directMembers.length, teamVolumeQualifiedUsd]);

  const userStatus = dynamicUserStatus ?? DEFAULT_STATUS;
  ...
  const totalDirectVolumeRaw = Number.isFinite(directVolumeUsd)
    ...
    : mockDirects.reduce((sum, d) => sum + (d.stakedUSD ?? 0), 0);
  ...
  const totalTeamVolumeRaw = Number.isFinite(teamVolumeQualifiedUsd)
    ...
    : mockDirects.reduce((sum, d) => sum + (d.teamVolume ?? 0), 0);
  const totalTeamVolume = normalizeUsdDisplay(totalTeamVolumeRaw);
  const totalTeamMembers = LEVEL_KEYS.reduce(
    (sum, level) => sum + (mockLevelData[level]?.length ?? 0),
    0
  );
  const activeMembers = LEVEL_KEYS.reduce(
    (sum, level) =>
      sum +
      (mockLevelData[level] || []).filter((m) => m.status === 'Active').length,
    0
  );
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="min-w-0">
                            ...
                            <NumberPopup
                              value={formatUSD((direct.stakedUSDDisplay ?? 0) * 1e8)}
                              label="Stake Amount"
                          ...
                          <div className="min-w-0">
                            ...
                            <NumberPopup
                              value={formatUSD((direct.teamVolumeDisplay ?? 0) * 1e8)}
                              label="Team Volume"
                      ...
                      {filteredData.map((member, idx) => (
                        <tr key={`${member.userId}-${idx}`}>
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm">
                            <NumberPopup
                              value={formatUSD((member.stakedUSDDisplay ?? member.stakedUSD ?? 0) * 1e8)}
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm">
                            <NumberPopup
                              value={formatUSD((normalizeUsdDisplay(member.totalEarned ?? 0)) * 1e8)}
                              label="Total Earned"
